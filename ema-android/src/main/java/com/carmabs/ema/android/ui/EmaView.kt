...
interface EmaView<S : EmaBaseState, VM : EmaViewModel<S, NS>, NS : EmaNavigationState> {
    ...
    fun initializeViewModel(fragmentActivity: FragmentActivity, fragment: Fragment? = null) {
        ...
        resultViewModel.resultEvent.observe(fragment ?: fragmentActivity, Observer(this::onResultHandled))

        vm.receiverAddedEvent.observe(fragment ?: fragmentActivity, Observer(this::onReceiverHandled))
     ...
     * @param result model
     */
    fun onResultHandled(result: EmaResultModel) {
        onResult(result)
    }

    /**
     * Called when view model trigger a receiver event
     * @param result model
     */
    fun onReceiverHandled(receiverList:HashMap<Int,EmaReceiverModel>) {
        onReceiverAdded(receiverList)
    }
     ...
     */
    fun onResult(emaResultModel: EmaResultModel)

    /**
     * Called when a receiver has been notified from view model
     * @param receiverId generated by view model
     */
    fun onReceiverAdded(allReceivers:HashMap<Int,EmaReceiverModel>)
